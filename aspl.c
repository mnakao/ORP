...
#pragma omp parallel for
    for(int i=0;i<_switches;i++)
      for(int j=i+1;j<_switches;j++)
        if(_distance[i*_switches+j] == 0 && (_B[i*_elements+(j/UINT64_BITS)] & (0x1ULL<<(j%UINT64_BITS))))
          _distance[i*_switches+j] = level;

    num *= _symmetries;
    if(num == (uint64_t)_switches*_switches) break;
...
#pragma omp parallel for reduction(+:local_sum)
  for(int i=0;i<_switches;i++){
    ...
    for(int j=i+1;j<_switches;j++)
      local_sum += (long)(_distance[i*_switches+j] + 2) * h_degree[i] * h_degree[j] * _symmetries;
  }
...
void ORP_Init_aspl_s(const int hosts, const int switches, const int radix, const int symmetries)
{
  ...
  if(_kind == ASPL_MATRIX){
    ...
    _distance = malloc(sizeof(int) * switches * switches);              // int _distance[switches][switches];
  }
  else{ // _kind == ASPL_BFS
